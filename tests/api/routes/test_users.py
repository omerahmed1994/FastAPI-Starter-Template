...
def test_create_user_new_email(
    ...
    with (
        ...
        created_user = r.json()
        user = users_service.get_user_by_email(session=db, email=username)
        assert user
...
def test_retrieve_users(
    ...
    users_service.create_user(session=db, user_create=user_in2)
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = users_service.create_user(session=db, user_create=user_in2)
