...
def test_authenticate_user(db: Session) -> None:
    ...
    user_in = UserCreate(email=email, password=password)
    user = users_service.create_user(session=db, user_create=user_in)
    authenticated_user = users_service.authenticate_user(session=db, email=email, password=password)
...
def test_not_authenticate_user(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user = users_service.authenticate_user(session=db, email=email, password=password)
    assert user is None
